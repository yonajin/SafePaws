...
if (isset($_POST['add_tip'])) {
  ...
  $sql = "INSERT INTO care_tips (name, content, image, status, date_published) 
          VALUES ('$name', '$content', '$image', '$status', NOW())";

  if (mysqli_query($conn, $sql)) {
    move_uploaded_file($_FILES['image']['tmp_name'], $target);
    echo "<script>alert('✅ Care Tip added successfully!'); window.location='care_tips.php';</script>";
  } else {
    echo "<script>alert('❌ Error adding tip: " . mysqli_error($conn) . "');</script>";
  }
}
?>
          ...
          if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
              echo "<tr>
                ...
                <td>
                  <a href='edit_tip.php?id={$row['id']}' class='btn btn-sm btn-warning'><i class='bi bi-pencil'></i></a>
                  <a href='delete_tip.php?id={$row['id']}' class='btn btn-sm btn-danger' onclick='return confirm(\"Delete this tip?\")'><i class='bi bi-trash'></i></a>
                </td>
              </tr>";
            }
          } else {
      ...
      <div class="modal-content border-0 shadow-lg rounded-4">
        ...
        <form method="POST" enctype="multipart/form-data">
          ...
          <div class="modal-footer bg-white">
            ...
            <button type="submit" name="add_tip" class="btn btn-add px-4">Save</button>
          </div>
        </form>
      </div>
    </div>
...
document.addEventListener("DOMContentLoaded", () => {
  const modalElement = document.getElementById("addTipModal");
  const openButton = document.getElementById("openAddTip");
  if (modalElement && openButton) {
    const addTipModal = new bootstrap.Modal(modalElement);
    openButton.addEventListener("click", () => addTipModal.show());
  }
});
